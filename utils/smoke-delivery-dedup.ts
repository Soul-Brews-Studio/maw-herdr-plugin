...
for (const parts of [['other','t','1','p'],['s','other','1','p'],['s','t','2','p'],['s','t','1','other']]) assert.notEqual(key, d.key(...parts as [string,string,string,string]));
const first = d.claim(key); assert.ok(first.complete); assert.ok(first.cancel);

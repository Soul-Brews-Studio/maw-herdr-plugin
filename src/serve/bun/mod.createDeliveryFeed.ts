...
export function createDeliveryFeed() {
  ...
  return {
    append(event: DeliveryEvent) {
      events.push({ ...event, text: truncate(event.text, 2000), from: truncate(event.from, 2000), to: truncate(event.to, 2000), ...(event.error === undefined ? {} : { error: truncate(event.error, 1000) }) });

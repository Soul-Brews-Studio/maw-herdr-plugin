...
export const HELP_VERBS = new Set(['ls', 'list', 'a', 'attach', 'wake', 'hey', 'peek', 'read', 'federation', 'fed', 'resolve', 'audit', 'clean', 'sync']);
...
const VALUE_FLAGS = {
  ...
  hey: new Set(['--session']),
  audit: new Set(['--idle', '--min-age', '--session']),
  clean: new Set(['--min-age', '--session']),
  sync: new Set(['--idle', '--session']),
};
